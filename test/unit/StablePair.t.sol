...
contract StablePairTest is BaseTest
{
    ...
    function testMintFee_DiffPlatformFees(uint256 aPlatformFee) public
    {
        ...
        // assert
        (uint256 lReserveC, uint256 lReserveD) = lPair.token0() == address(_tokenC) ? (lReserve0, lReserve1) : (lReserve1, lReserve0);
    ...
    function testSwap_VerySmallLiquidity(uint256 aAmtBToMint, uint256 aAmtCToMint, uint256 aSwapAmt) public
    {
        ...
        uint256 lAmtOut = lPair.swap(lPair.token0() == address(_tokenB) ? int256(lSwapAmt) : -int256(lSwapAmt), true, address(this), bytes(""));
    ...
    function testSwap_VeryLargeLiquidity(uint256 aSwapAmt) public
    {
        ...
        uint256 lAmtOut = lPair.swap(lPair.token0() == address(_tokenC) ? int256(lSwapAmt) : -int256(lSwapAmt), true, address(this), bytes(""));
    ...
    function testSwap_DiffSwapFees(uint256 aSwapFee) public
    {
        ...
        uint256 lAmtOut = lPair.swap(
            lPair.token0() == address(_tokenD) ? int256(lSwapAmt) : -int256(lSwapAmt),
            true,
            address(this),
            bytes("")
        );
    ...
    function testSwap_DiffAs(uint256 aAmpCoeff, uint256 aSwapAmt, uint256 aMintAmt) public
    {
        ...
        // act
        _tokenD.mint(address(lPair), lSwapAmt);
        lPair.swap(lPair.token0() == address(_tokenD) ? int256(lSwapAmt) : -int256(lSwapAmt), true, address(this), bytes(""));
    ...
    function testOracle_SimplePrices() external
    {
        ...
        assertEq(lObs1.logAccRawPrice, LogCompression.toLowResLog(1e18) * 10 + LogCompression.toLowResLog(lSpotPrice1) * 10, "2");
        assertEq(
