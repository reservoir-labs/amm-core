...
contract StablePair is ReservoirPair {
    ...
    function rampA(uint64 futureARaw, uint64 futureATime) external onlyFactory {
        ...
        uint duration = futureATime - block.timestamp;
        ...
        // losing precision
        uint dailyRate = futureAPrecise > currentAPrecise
    ...
    function _nonOptimalMintFee(uint _amount0, uint _amount1, uint _reserve0, uint _reserve1)
        internal
        view
        returns (uint token0Fee, uint token1Fee)
    {
        if (_reserve0 == 0 || _reserve1 == 0) return (0, 0);
        uint amount1Optimal = (_amount0 * _reserve1) / _reserve0;
    ...
    function mint(address to) public nonReentrant returns (uint liquidity) {
        ...
        (uint balance0, uint balance1) = _balance();

        uint newLiq = _computeLiquidity(balance0, balance1);
        uint amount0 = balance0 - _reserve0;
        uint amount1 = balance1 - _reserve1;

        (uint fee0, uint fee1) = _nonOptimalMintFee(amount0, amount1, _reserve0, _reserve1);
        _reserve0 += uint112(fee0);
        _reserve1 += uint112(fee1);

        (uint _totalSupply, uint oldLiq) = _mintFee(_reserve0, _reserve1);
    ...
    function burn(address to) public nonReentrant returns (uint amount0, uint amount1) {
        _syncManaged();

        (uint _reserve0, uint _reserve1,) = getReserves();
        uint liquidity = balanceOf[address(this)];

        (uint _totalSupply,) = _mintFee(_reserve0, _reserve1);
    ...
    function _balance() internal view returns (uint balance0, uint balance1) {
        balance0 = _totalToken0();
        balance1 = _totalToken1();
    }

    function _getAmountOut(uint amountIn, uint _reserve0, uint _reserve1, bool token0In) internal view returns (uint) {
    ...
    function _getAmountIn(uint amountOut, uint _reserve0, uint _reserve1, bool token0Out)
        internal
        view
        returns (uint)
    {
    ...
    function _computeLiquidity(uint _reserve0, uint _reserve1) internal view returns (uint liquidity) {
        unchecked {
            uint adjustedReserve0 = _reserve0 * token0PrecisionMultiplier;
            uint adjustedReserve1 = _reserve1 * token1PrecisionMultiplier;
            liquidity = StableMath._computeLiquidityFromAdjustedBalances(adjustedReserve0, adjustedReserve1, _getNA());
        }
    }

    function _mintFee(uint _reserve0, uint _reserve1) internal returns (uint _totalSupply, uint d) {
        _totalSupply = totalSupply;
        uint _dLast = lastInvariant;
        if (_dLast != 0) {
            ...
            if (d > _dLast) {
                ...
                uint _platformFee = platformFee;
                uint numerator = _totalSupply * (d - _dLast) * _platformFee;
                uint denominator = (FEE_ACCURACY - _platformFee) * d + _platformFee * _dLast;
    ...
    function _getNA() internal view returns (uint) {
    ...
    function getAmountOut(address tokenIn, uint amountIn) public view returns (uint finalAmountOut) {
        (uint _reserve0, uint _reserve1,) = getReserves();
    ...
    function getVirtualPrice() public view returns (uint virtualPrice) {
        (uint _reserve0, uint _reserve1,) = getReserves();
        uint d = _computeLiquidity(_reserve0, _reserve1);
        virtualPrice = (d * (uint(10) ** decimals)) / totalSupply;
    }
        ...
        override
    {
        ...
        (uint currRawPrice, int112 currLogRawPrice) = StableOracleMath.calcLogPrice(
